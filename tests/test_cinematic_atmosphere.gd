...
func _run() -> void:
	...
	failed += _test_main_applies_on_ready()
	if failed > 0:
...
func _test_environment_look() -> int:
	var env: Environment = MainScript.make_cinematic_environment()
...
func _test_sun_style() -> int:
	...
	MainScript.style_directional_light(sun)
...
func _test_main_applies_on_ready() -> int:
	var src := FileAccess.get_file_as_string("res://scripts/main.gd")
	if src.find("_apply_cinematic_atmosphere") < 0 or src.find("make_cinematic_environment") < 0:
		push_error("main.gd should build the WorldEnvironment in code")
		return 1
	if src.find("WorldEnvironment.new()") < 0:
		push_error("main.gd should create a WorldEnvironment node")
		return 1
